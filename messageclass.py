...
class Message:
     ...
     def get_function_id(self):
          ...
          if (ser.in_waiting > 0):
               ...
               self.accept_time = datetime.now().time()
               time.sleep(t)
          if (ser.in_waiting > 0):
               self.bytes.append(ser.read())
          
          self.function_id = self.bytes[0]
     ...
     def print_message(self):
          logger.info("message: %s, acception time: %s", " ".join(map(str, bytes)), self.accept_time )
          loggerf.info("message: %s, acception time: %s", " ".join(map(str, bytes)), self.accept_time )
          self.bytes.clear()
